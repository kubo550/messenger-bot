...
export class ScheduleResponder extends MessengerResponder {
    ...
    public async handlePayload() {
        if (this.payload === 'SCHEDULE') {
            ...
            const message = schedule.map(({date, day, subject, teacher, room, from}) =>
                `${onlyMonthAndDay(date)} ${day} ${from} ${subject}  ${teacher || ''} ${generateClass(room)}`).join('\n');
            const firstMessage = this.genTextMessage(`W tym tygodniu masz ${schedule.length} zajęć:`);
            ...
            const lessonShortName = this.payload.replace('SCHEDULE_TERMINAL_', '').toLowerCase() as LessonShortName;
            const scheduleClient = new ScheduleApiClient('190201');
            const schedule = await scheduleClient.getLessonsTerminal(lessonShortName);
            return [
                this.genTextMessage(`Łącznie ${uppercaseFirstLetter(lessonShortName)} masz ${schedule.length} zajęć:`),
                this.genTextMessage(schedule.map(({day, date, teacher, room, from}) =>
                    `${onlyMonthAndDay(date)} ${day} ${from} ${teacher || ''} ${generateClass(room)}`).join('\n'))
            ];
        }
    }
