...
export type LessonFullName =
  | 'Pracownia programowania II'
  | 'Ekonomia'
...
export class ScheduleApiClient {
  ...
  public async getLessonsTerminal(
    lessonName: LessonShortName,
  ): Promise<Lesson[]> {
    const lessons = await this.fetchSchedule(this.generateScheduleUrl('2'));
    return lessons
      .filter(isFutureLesson)
      .filter(isThisLesson(getLessonFullName(lessonName)));
  }
}
